import { FETCH_TASK_REQUEST, CREATE_TASK_RESPONSE, DELETE_EMPLOYEE_REQUEST, DELETE_EMPLOYEE_RESPONSE, ME_REQUEST, ME_RESPONSE, READ_EMPLOYEE_REQUEST, READ_EMPLOYEE_RESPONSE, REGISTER_EMPLOYEE_REQUEST, REGISTER_EMPLOYEE_RESPONSE, RESTORE_EMPLOYEE_REQUEST, RESTORE_EMPLOYEE_RESPONSE, UPDATE_EMPLOYEE_REQUEST, UPDATE_EMPLOYEE_RESPONSE, UPDATE_PROFILE_REQUEST, FETCH_TASK_RESPONSE, CREATE_TASK_REQUEST, UPDATE_PROFILE_RESPONSE, UPDATE_TASK_REQUEST, UPDATE_TASK_RESPONSE, UPDATE_TASK_DETAILS_REQUEST, UPDATE_TASK_DETAILS_RESPONSE, DELETE_TASK_REQUEST, DELETE_TASK_RESPONSE } from "@repo/types"
...
import { registerTemplate } from "../utils/register.templete"
...
export const registerEmployee = async (req: Request<{}, {}, REGISTER_EMPLOYEE_REQUEST>, res: Response<REGISTER_EMPLOYEE_RESPONSE>) => {
    try {
        ...
        adminProfileUpload(mreq, res, async err => {
            ...
            await sendEmail({
                email,
                subject: "TaskForge Registration",
        ...
        console.log(error)
        res.status(500).json({ message: "unable to register server error" })
    }
}
...
export const readEmployee = async (req: Request<{}, {}, READ_EMPLOYEE_REQUEST>, res: Response<READ_EMPLOYEE_RESPONSE>) => {
    try {
        ...
        res.status(200).json({ message: "Read Employee Successfully", result })
    } catch (error) {
        console.log(error)
        res.status(500).json({ message: "unable to register server error" })
    }
...
export const restoreEmployee = async (req: Request<{ eid: number }, {}, RESTORE_EMPLOYEE_REQUEST>, res: Response<RESTORE_EMPLOYEE_RESPONSE>) => {
    try {
        ...
        res.status(200).json({ message: "Restore Employee Successfully" })
    } catch (error) {
        console.log(error)
        res.status(500).json({ message: "unable to restore employee server error" })
    }
}
...
export const createTask = async (req: Request<{}, {}, CREATE_TASK_REQUEST>, res: Response<CREATE_TASK_RESPONSE>) => {
    try {
        ...
        console.log(error)
        res.status(500).json({ message: "unable to create task server error" })
    }
}
...
export const fetchTask = async (req: Request<{}, {}, FETCH_TASK_REQUEST>, res: Response<FETCH_TASK_RESPONSE>) => {
    try {
        ...
        res.status(201).json({ message: "Fetch Task Successfully", result })

    } catch (error) {
        console.log(error)
        res.status(500).json({ message: "unable to fetch task server error" })
    }
}
...
export const updateTask = async (req: Request<{}, {}, UPDATE_TASK_REQUEST>, res: Response<UPDATE_TASK_RESPONSE>) => {
    try {
        ...
        console.log(error)
        res.status(500).json({ message: "unable to update task server error" })
    }
}
...
export const updateTaskDetails = async (
    ...
    try {
        ...
        taskHeroUpload(mreq, res, async err => {
            ...
            res.status(201).json({ message: "Update Task Details Successfully" })
        })
    } catch (error) {
        console.log(error)
        res.status(500).json({ message: "unable to update task details server error" })
    }
}
...
export const deleteTask = async (
    ...
    try {
        ...
        console.log(error)
        res.status(500).json({ message: "unable to delete task server error" })
    }
}